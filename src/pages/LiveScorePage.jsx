...
export default function LiveScorePage() {
  ...
  // When scores refresh from API, update the current match view
  useEffect(() => {
    if (!match?.id) return;
    ...
    <div style={{ background: "var(--bg)", minHeight: "100vh" }}>
      ...
      <div style={{ display: "flex", maxWidth: 1100, margin: "0 auto", padding: "24px 20px", gap: 24 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <button onClick={() => navigate("home")} className="btn btn-ghost" style={{ marginBottom: 20, fontSize: 13, display: "flex", alignItems: "center", gap: 6 }}>
            ← Back to highlights
          </button>

          {match ? (
            <div>
              <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, padding: "36px 28px", textAlign: "center", marginBottom: 20 }}>
                <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 10, marginBottom: 28 }}>
                  ...
                  <span style={{
                    ...
                    color: statusColor(match.status), letterSpacing: "0.06em",
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 24, marginBottom: 28 }}>
                  <div style={{ flex: 1, textAlign: "right" }}>
                    <div style={{ fontSize: 22, fontWeight: 700, color: "var(--text)", fontFamily: "'Bebas Neue', sans-serif", letterSpacing: "0.04em" }}>{match.home_team}</div>
                  ...
                  <div style={{ background: "var(--surface2)", border: "1px solid var(--border)", borderRadius: 14, padding: "18px 32px", minWidth: 120, flexShrink: 0, textAlign: "center" }}>
                    ...
                    {match.status === "live" && (
                      <div style={{ fontSize: 13, color: "var(--green-light)", marginTop: 8, fontWeight: 600 }}>{match.minute}'</div>
                    )}
                  </div>

                  <div style={{ flex: 1, textAlign: "left" }}>
                    <div style={{ fontSize: 22, fontWeight: 700, color: "var(--text)", fontFamily: "'Bebas Neue', sans-serif", letterSpacing: "0.04em" }}>{match.away_team}</div>
                ...
                {match.status === "live" && (
                  <div style={{ fontSize: 12, color: "var(--text-muted)", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
                    <span className="live-dot" /> Updating in real time · refreshes every 60s
                  </div>
                )}
        ...
        <div style={{ width: 290, flexShrink: 0 }}>
          <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "var(--text-muted)", marginBottom: 14 }}>Today's Fixtures</div>

          {loading ? (
            <div style={{ color: "var(--text-muted)", fontSize: 13 }}>Fetching scores...</div>
          ) : Object.keys(groupedScores).length === 0 ? (
            <div style={{ fontSize: 12, color: "var(--text-muted)", textAlign: "center", padding: "20px 0" }}>No matches today</div>
          ) : (
            Object.entries(groupedScores).map(([league, matches]) => (
              <div key={league} style={{ marginBottom: 18 }}>
                <div style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", color: "var(--text-dim)", marginBottom: 8, paddingLeft: 2 }}>{league}</div>
                  ...
                  {matches.map(m => (
                    ...
                    >
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <span style={{ color: "var(--text-sec)", fontWeight: 500, fontSize: 12, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{m.home_team}</span>
                        <span style={{ color: "var(--gold)", fontWeight: 700, fontSize: 13, padding: "0 8px", flexShrink: 0 }}>{m.home_score}–{m.away_score}</span>
                        <span style={{ color: "var(--text-sec)", fontWeight: 500, fontSize: 12, flex: 1, textAlign: "right", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{m.away_team}</span>
                      </div>
