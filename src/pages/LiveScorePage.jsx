...
function normalise(f) {
  ...
  return {
    ...
    events:      f.events || [],
    homeScorers: (f.events||[]).filter(e=>e.type==="Goal"&&e.team?.id===homeId).map(e=>`${e.player?.name} ${e.time?.elapsed}'`),
    awayScorers: (f.events||[]).filter(e=>e.type==="Goal"&&e.team?.id!==homeId).map(e=>`${e.player?.name} ${e.time?.elapsed}'`),
  };
}
...
function StatusBadge({ match }) {
  ...
  if (match.status === "ft") return <div style={{ color:"var(--text-muted)", fontSize:11, fontWeight:600, minWidth:50, textAlign:"center" }}>FT</div>;
...
function MatchDetail({ match, onClose }) {
  ...
  return (
    <div>
      <button onClick={onClose} className="btn btn-ghost" style={{ margin:"12px 16px 0", fontSize:12, padding:"5px 10px" }}>← Back</button>
...
export default function LiveScorePage() {
  ...
  const fetchMatches = useCallback(async (activeTab) => {
    if (!API_KEY) { setError("API key missing — add REACT_APP_FOOTBALL_API_KEY to Vercel env vars."); setLoading(false); return; }
    try {
      const url  = activeTab==="live" ? `${BASE_URL}/fixtures?live=all` : `${BASE_URL}/fixtures?date=${TODAY}`;
  ...
  useEffect(()=>{
    setLoading(true); setMatches([]); setSelected(null);
    fetchMatches(tab);
  ...
  return (
    <div style={{ background:"var(--bg)", minHeight:"100vh", display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ background:"var(--surface)", borderBottom:"1px solid var(--border)", padding:"0 20px", display:"flex" }}>
        {TABS.map(t=>(
          <button key={t.id} onClick={()=>setTab(t.id)} style={{
            ...
            transition:"all 0.15s", cursor:"pointer", display:"flex", alignItems:"center", gap:6,
          }}>
            {t.id==="live"&&tab==="live"&&liveCount>0&&<span style={{ width:6,height:6,borderRadius:"50%",background:"#22c55e",boxShadow:"0 0 6px #22c55e" }}/>}
            {t.label}
          </button>
        ))}
      </div>
      ...
      <div style={{ flex:1, display:"flex", overflow:"hidden" }}>
        ...
        {/* Match list */}
        <div style={{ flex: selected?"0 0 55%":"1", overflowY:"auto", maxHeight:"calc(100vh - 160px)", borderRight: selected?"1px solid var(--border)":"none" }}>
          {loading ? (
            <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"80px 20px", gap:16, color:"var(--text-muted)" }}>
              ...
              <span style={{ fontSize:13 }}>Fetching scores from all leagues worldwide...</span>
            </div>
          ) : error ? (
            <div style={{ textAlign:"center", padding:"60px 20px", color:"var(--text-muted)" }}>
              <div style={{ fontSize:36, marginBottom:12, opacity:0.2 }}>⚠️</div>
              <div style={{ fontSize:14, marginBottom:6 }}>{error}</div>
              <div style={{ fontSize:12, color:"var(--text-dim)" }}>Go to Vercel → Settings → Environment Variables and add REACT_APP_FOOTBALL_API_KEY</div>
            </div>
          ) : matches.length===0 ? (
            <div style={{ textAlign:"center", padding:"80px 20px", color:"var(--text-muted)" }}>
              ...
              <div style={{ fontSize:12, color:"var(--text-dim)" }}>
                {tab==="live"?"Most matches kick off evenings & weekends. Check Fixtures tab for today's schedule.":"Try another tab above"}
              </div>
            </div>
          ) : (
